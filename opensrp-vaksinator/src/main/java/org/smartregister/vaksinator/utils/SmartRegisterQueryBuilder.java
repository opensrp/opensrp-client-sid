...
public class SmartRegisterQueryBuilder {
    ...
    public String mainCondition(String tablename, String condition) {
        Selectquery = Selectquery + (!condition.isEmpty() ? " WHERE " + setTablenameToCondition(tablename,condition) + " " : "");
        return Selectquery;
    }
    ...
    public String countQueryFts(String tablename, String searchJoinTable, String mainCondition,
        ...
        if (StringUtils.isNotBlank(searchJoinTable) && StringUtils.isNotBlank(searchFilter)) {
            String query = "SELECT " + CommonFtsObject.idColumn + " FROM " + CommonFtsObject
                    .searchTableName(tablename) + phraseClauseWithTable(tablename, searchJoinTable, mainCondition,
                    searchFilter);
            return query;
        }
        String query = "SELECT " + CommonFtsObject.idColumn + " FROM " + CommonFtsObject
    ...
    private String phraseClauseWithTable(String tablename, String mainCondition, String phrase) {
        if (StringUtils.isNotBlank(phrase)) {
            String phraseClause =
                    " WHERE " + setTablenameToCondition(CommonFtsObject.searchTableName(tablename), mainConditionClause(mainCondition)) + CommonFtsObject.phraseColumn
                            + matchPhrase(phrase);
            return phraseClause;
        } else if (StringUtils.isNotBlank(mainCondition)) {
            return " WHERE " + setTablenameToCondition(CommonFtsObject.searchTableName(tablename), mainCondition);
        }
        return "";
    }

    private String phraseClauseWithTable(String tablename, String joinTable, String mainCondition, String phrase) {
        String phraseClause =
                " WHERE " + setTablenameToCondition(CommonFtsObject.searchTableName(tablename), mainConditionClause(mainCondition)) + CommonFtsObject.phraseColumn
