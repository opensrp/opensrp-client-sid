package org.smartregister.vaksinator.facial.repository;
...
public class ImageRepository extends BaseRepository {
    ...
    public void add(ProfileImage profileImage) {
        try {
            ...
            profileImage.setUpdatedAt(Calendar.getInstance().getTimeInMillis());
...
//            abc = ((c != null && c.getCount() > 0))? c.getLong(c.getColumnIndex(ID_COLUMN)) : 0L ;
    ...
    public ProfileImage findByBaseEntityId(String baseEntityId){
        ...
        Cursor cursor = null;
        try {
            cursor = getRepository().getReadableDatabase().query(PHOTO_TABLE_NAME, PHOTO_TABLE_COLUMNS, BASE_ENTITY_ID_COLUMN + " = ?", new String[]{baseEntityId}, null, null, null, null);
            List<ProfileImage> profileImages = readAllFacials(cursor);
            if (!profileImages.isEmpty()) profileImage = profileImages.get(0);
