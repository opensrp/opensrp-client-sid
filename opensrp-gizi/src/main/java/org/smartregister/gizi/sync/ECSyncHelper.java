...
import static util.GiziConstants.LAST_CHECK_TIMESTAMP;
import static util.GiziConstants.LAST_SYNC_TIMESTAMP;
import static util.GiziConstants.LAST_VIEWS_SYNC_TIMESTAMP;
...
public class ECSyncHelper implements PrefsHelper {
    ...
    public static ECSyncHelper getInstance(Context context) {
        if (instance == null) {
            instance = new ECSyncHelper(context, org.smartregister.gizi.application.GiziApplication.getInstance().getEventClientRepository());
        }
        return instance;
    }
    ...
    public JSONObject fetchAsJsonObject(String filter, String filterValue) throws Exception {
        try {
            HTTPAgent httpAgent = org.smartregister.gizi.application.GiziApplication.getInstance().getContext().getHttpAgent();
            String baseUrl = org.smartregister.gizi.application.GiziApplication.getInstance().getContext().
