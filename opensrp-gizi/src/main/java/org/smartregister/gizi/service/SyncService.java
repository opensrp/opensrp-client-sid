...
public class SyncService extends Service {
    ...
    private void pushECToServer() {
        IndonesiaECRepository db = GiziApplication.getInstance().indonesiaECRepository();
    ...
    private void pushPhotoToServer(){
        ...
        List<ProfileImage> pendingImages = GiziApplication.getInstance().imageRepository().getAllUnsyncImages();
        ...
        for (ProfileImage image : pendingImages){
            ...
            image1.setAnmId(GiziApplication.getInstance().context().allSharedPreferences().fetchRegisteredANM());
            ...
            GiziApplication.getInstance().imageRepository().add(image);
        }
    }
    ...
    private void pullPhotoFromServer(){
        ImageRepository imageRepo = GiziApplication.getInstance().imageRepository();
        String baseUrl = GiziApplication.getInstance().context().configuration().dristhiBaseURL();
        if (baseUrl.endsWith(context.getString(R.string.url_separator))) {
            baseUrl = baseUrl.substring(0, baseUrl.lastIndexOf(context.getString(R.string.url_separator)));
        }
    ...
    private int downloadPhoto(String fileURL, String saveDir) throws Exception {
        ...
        //add request header
        httpConn.setRequestProperty("username", GiziApplication.getInstance().context().allSharedPreferences().fetchRegisteredANM());
        httpConn.setRequestProperty("password", GiziApplication.getInstance().context().allSettings().fetchANMPassword());
