...
import org.smartregister.gizi.model.Result;
...
public class RenderPositiveResultsCardHelper extends BaseRenderHelper {
    ...
    private void initializeRenderLayout(InitializeRenderParams params) {
        ...
        GiziSpannableStringBuilder stringBuilder = getConstructedStringBuilder(testResults, params);
    ...
    private Map<String, Result> getLatestResults(InitializeRenderParams params) {
        return ((ResultsRepository) repository).getLatestResultsAll(params.view.getTag(R.id.BASE_ENTITY_ID).toString(), params.isIntreatment ? Long.valueOf(params.extra.get(GiziConstants.KEY.BASELINE)) : null, true);
    }

    private Map<String, Result> getBaselineTestResults(InitializeRenderParams params) {
        return ((ResultsRepository) repository).getLatestResultsAll(params.view.getTag(R.id.BASE_ENTITY_ID).toString(), Long.valueOf(params.extra.get(GiziConstants.KEY.BASELINE)), false);
    }

    private GiziSpannableStringBuilder getResultPrefixBuilder(InitializeRenderParams params, GiziSpannableStringBuilder stringBuilder, Date dateTestGiven) {
        if (params.isIntreatment && !params.isBaseline) {
            Date dateTreatmentInitiatied = org.smartregister.util.Utils.toDate(params.extra.get(GiziConstants.KEY.TREATMENT_INITIATION_DATE), true);
    ...
    private GiziSpannableStringBuilder getConstructedStringBuilder(Map<String, Result> testResults, InitializeRenderParams params) {
        ...
        for (Map.Entry<String, Result> entry : testResults.entrySet()) {
            if (testResults.containsKey(entry.getKey()) && !entry.getKey().equals(GiziConstants.RESULT.RIF_RESULT)) {
                stringBuilder = getResultPrefixBuilder(params, stringBuilder, testResults.get(entry.getKey()).getDate());
                switch (entry.getKey()) {
                    case GiziConstants.RESULT.MTB_RESULT:
                        if (testResults.get(GiziConstants.RESULT.RIF_RESULT) != null) {
                            testResultsStringBuilderHelper.getXpertResultStringBuilder(ImmutableMap.of(entry.getKey(), testResults.get(entry.getKey()).getValue1(), GiziConstants.RESULT.RIF_RESULT, testResults.get(GiziConstants.RESULT.RIF_RESULT).getValue1()), stringBuilder, false);
                        ...
                        break;
                    case GiziConstants.RESULT.TEST_RESULT:
                        ...
                        break;
                    case GiziConstants.RESULT.XRAY_RESULT:
                        ...
                        break;
                    case GiziConstants.RESULT.CULTURE_RESULT:
