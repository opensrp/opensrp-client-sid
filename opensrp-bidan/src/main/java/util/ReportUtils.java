...
import org.smartregister.bidan.domain.Report;
...
public class ReportUtils {
    ...
    public static void createReport(Context context, List<ReportHia2Indicator> hia2Indicators, Date month, String reportType) {
        try {
            ...
            String providerId = BidanApplication.getInstance().context().allSharedPreferences().fetchRegisteredANM();
            String locationId = BidanApplication.getInstance().context().allSharedPreferences().getPreference(PathConstants.CURRENT_LOCATION_ID);
