...
import org.smartregister.bidan.model.Result;
...
public class RenderPositiveResultsCardHelper extends BaseRenderHelper {
    ...
    private void initializeRenderLayout(InitializeRenderParams params) {
        ...
        BidanSpannableStringBuilder stringBuilder = getConstructedStringBuilder(testResults, params);
    ...
    private Map<String, Result> getLatestResults(InitializeRenderParams params) {
        return ((ResultsRepository) repository).getLatestResultsAll(params.view.getTag(R.id.BASE_ENTITY_ID).toString(), params.isIntreatment ? Long.valueOf(params.extra.get(BidanConstants.KEY.BASELINE)) : null, true);
    }

    private Map<String, Result> getBaselineTestResults(InitializeRenderParams params) {
        return ((ResultsRepository) repository).getLatestResultsAll(params.view.getTag(R.id.BASE_ENTITY_ID).toString(), Long.valueOf(params.extra.get(BidanConstants.KEY.BASELINE)), false);
    }

    private BidanSpannableStringBuilder getResultPrefixBuilder(InitializeRenderParams params, BidanSpannableStringBuilder stringBuilder, Date dateTestGiven) {
        if (params.isIntreatment && !params.isBaseline) {
            Date dateTreatmentInitiatied = org.smartregister.util.Utils.toDate(params.extra.get(BidanConstants.KEY.TREATMENT_INITIATION_DATE), true);
    ...
    private BidanSpannableStringBuilder getConstructedStringBuilder(Map<String, Result> testResults, InitializeRenderParams params) {
        ...
        for (Map.Entry<String, Result> entry : testResults.entrySet()) {
            if (testResults.containsKey(entry.getKey()) && !entry.getKey().equals(BidanConstants.RESULT.RIF_RESULT)) {
                stringBuilder = getResultPrefixBuilder(params, stringBuilder, testResults.get(entry.getKey()).getDate());
                switch (entry.getKey()) {
                    case BidanConstants.RESULT.MTB_RESULT:
                        if (testResults.get(BidanConstants.RESULT.RIF_RESULT) != null) {
                            testResultsStringBuilderHelper.getXpertResultStringBuilder(ImmutableMap.of(entry.getKey(), testResults.get(entry.getKey()).getValue1(), BidanConstants.RESULT.RIF_RESULT, testResults.get(BidanConstants.RESULT.RIF_RESULT).getValue1()), stringBuilder, false);
                        ...
                        break;
                    case BidanConstants.RESULT.TEST_RESULT:
                        ...
                        break;
                    case BidanConstants.RESULT.XRAY_RESULT:
                        ...
                        break;
                    case BidanConstants.RESULT.CULTURE_RESULT:
