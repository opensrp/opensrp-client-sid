...
import org.smartregister.bidan.domain.ReportHia2Indicator;
...
//        monthlyTalliesRepository = BidanApplication.getInstance().monthlyTalliesRepository();
    ...
    private void generateDailyHia2Indicators() {
        try {
            ...
            SQLiteDatabase db = BidanApplication.getInstance().getRepository().getWritableDatabase();
            //get previous dates if shared preferences is null meaning reports for previous months haven't been generated
            String lastProcessedDate = BidanApplication.getInstance().context().allSharedPreferences().getPreference(HIA2Service.HIA2_LAST_PROCESSED_DATE);
            ...
            for (Map<String, String> dates : reportDates) {
                ...
                BidanApplication.getInstance().context().allSharedPreferences().savePreference(HIA2Service.HIA2_LAST_PROCESSED_DATE, updatedAt);
            }
        } catch (Exception e) {
            Log.e(TAG, e.getMessage(), e);
        }
    ...
    private Long countDaysAfterDueDate(Vaccine vaccine) {
        ...
        CommonRepository commonRepository = BidanApplication.getInstance().context().commonrepository(PathConstants.CHILD_TABLE_NAME);
