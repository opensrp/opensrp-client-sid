...
import org.smartregister.bidan.wrapper.VaccineViewRecordUpdateWrapper;
import org.smartregister.bidan.wrapper.WeightViewRecordUpdateWrapper;
...
public class AnakSmartClientsProvider implements SmartRegisterCLientsProviderForCursorAdapter {
    ...
    public AnakSmartClientsProvider(Context context, View.OnClickListener onClickListener,
    ...
    public void getView(Cursor cursor, SmartRegisterClient client, final View convertView) {
        ...
        fillValue((TextView) convertView.findViewById(R.id.child_zeir_id), getValue(pc.getColumnmaps(), BidanConstants.KEY.ZEIR_ID, false));
    ...
    private void updateRecordVaccination(VaccineViewRecordUpdateWrapper updateWrapper) {
        ...
        if (nv != null) {
            DateTime dueDate = (DateTime) nv.get(BidanConstants.KEY.DATE);
            VaccineRepo.Vaccine vaccine = (VaccineRepo.Vaccine) nv.get(BidanConstants.KEY.VACCINE);
            stateKey = VaccinateActionUtils.stateKey(vaccine);
            if (nv.get(BidanConstants.KEY.ALERT) == null) {
                state = State.NO_ALERT;
            } else if (((Alert) nv.get(BidanConstants.KEY.ALERT)).status().value().equalsIgnoreCase(BidanConstants.KEY.NORMAL)) {
                state = State.DUE;
            } else if (((Alert) nv.get(BidanConstants.KEY.ALERT)).status().value().equalsIgnoreCase(BidanConstants.KEY.UPCOMING)) {
            ...
            } else if (((Alert) nv.get(BidanConstants.KEY.ALERT)).status().value().equalsIgnoreCase(BidanConstants.KEY.URGENT)) {
                state = State.OVERDUE;
            } else if (((Alert) nv.get(BidanConstants.KEY.ALERT)).status().value().equalsIgnoreCase(BidanConstants.KEY.EXPIRED)) {
                state = State.EXPIRED;
            }
        } else {
            state = State.WAITING;
        ...
        if (state.equals(State.FULLY_IMMUNIZED)) {
            ...
            if (StringUtils.isNotBlank(stateKey) && (StringUtils.containsIgnoreCase(stateKey, BidanConstants.KEY.WEEK) || StringUtils.containsIgnoreCase(stateKey, BidanConstants.KEY.MONTH)) && !updateWrapper.getVaccines().isEmpty()) {
                Vaccine vaccine = updateWrapper.getVaccines().isEmpty() ? null : updateWrapper.getVaccines().get(updateWrapper.getVaccines().size() - 1);
                String previousStateKey = VaccinateActionUtils.previousStateKey(BidanConstants.KEY.CHILD, vaccine);
        ...
        if (commonRepository != null) {
            ...
            if (commonPersonObject == null) { //Out of area -- doesn't exist in local database
                if (isWeightRecord) {
                    ...
                    String zeirId = getValue(pc.getColumnmaps(), BidanConstants.KEY.ZEIR_ID, false);
                    ...
                    String motherBaseEntityId = getValue(pc.getColumnmaps(), BidanConstants.KEY.MOTHER_BASE_ENTITY_ID, false);
    ...
    private class VaccinationAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            List<Alert> alerts = alertService.findByEntityIdAndAlertNames(entityId, VaccinateActionUtils.allAlertNames(BidanConstants.KEY.CHILD));

            Map<String, Date> recievedVaccines = receivedVaccines(vaccines);

            List<Map<String, Object>> sch = generateScheduleList(BidanConstants.KEY.CHILD, new DateTime(dobString), recievedVaccines, alerts);
