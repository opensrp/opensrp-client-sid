...
public class BidanSmartRegisterQueryBuilder {
    private static final String TAG = BidanSmartRegisterQueryBuilder.class.getName();
    String Selectquery;

    public BidanSmartRegisterQueryBuilder(String selectquery) {
        Selectquery = selectquery;
    }

    public BidanSmartRegisterQueryBuilder() {
    }
    ...
    public String searchQueryFts(String tablename, String searchJoinTable, String mainCondition,
        ...
        if (StringUtils.isNotBlank(searchJoinTable) && StringUtils.isNotBlank(searchFilter)) {
            Log.d(TAG, "searchQueryFts: StringUtils.isNotBlank(searchJoinTable) && StringUtils.isNotBlank(searchFilter)");
        ...
        Log.d(TAG, "searchQueryFts: else");
        String query = "SELECT " + CommonFtsObject.idColumn + " FROM " + CommonFtsObject
                .searchTableName(tablename) + phraseClauseTable(CommonFtsObject
                .searchTableName(tablename), mainCondition, searchFilter)
    ...
    private String phraseClauseTable(String tablename, String mainCondition, String phrase) {
        if (StringUtils.isNotBlank(phrase)) {
            String phraseClause =
                    " WHERE " +tablename+"."+ mainConditionClause(mainCondition) + CommonFtsObject.phraseColumn
                            + matchPhrase(phrase);
            return phraseClause;
        } else if (StringUtils.isNotBlank(mainCondition)) {
            return " WHERE " +tablename+"." + mainCondition;
        }
        return "";
    }
