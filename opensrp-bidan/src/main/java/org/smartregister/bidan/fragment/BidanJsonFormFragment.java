...
import org.smartregister.bidan.activity.BidanJsonFormActivity;
import org.smartregister.bidan.application.BidanApplication;
import org.smartregister.bidan.interactors.PathJsonFormInteractor;
import org.smartregister.bidan.provider.MotherLookUpSmartClientsProvider;
import org.smartregister.bidan.viewstates.PathJsonFormFragmentViewState;
...
public class BidanJsonFormFragment extends JsonFormFragment {

    private Snackbar snackbar = null;
    private AlertDialog alertDialog = null;
    private boolean lookedUp = false;

    public static BidanJsonFormFragment getFormFragment(String stepName) {
        BidanJsonFormFragment jsonFormFragment = new BidanJsonFormFragment();
        ...
        if (item.getItemId() == com.vijay.jsonwizard.R.id.action_save) {
            ...
            try {
                if (object.getString("title").contains("Stock Issued") || object.getString("title").contains("Stock Received") || object.getString("title").contains("Stock Loss/Adjustment")) {
                    balanceCheck = ((BidanJsonFormActivity) getActivity()).checkIfBalanceNegative();
                }

                if (object.getString("title").contains("Record out of catchment area service")) {
                    fillFormCheck = ((BidanJsonFormActivity) getActivity()).checkIfAtLeastOneServiceGiven();
                }
            } catch (Exception e) {
                Log.e(getClass().getName(), e.toString(), e);
            }
