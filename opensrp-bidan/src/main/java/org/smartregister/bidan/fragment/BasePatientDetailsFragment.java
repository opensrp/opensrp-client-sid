...
import org.smartregister.bidan.helper.FormOverridesHelper;
import org.smartregister.bidan.helper.view.RenderBMIHeightChartCardHelper;
import org.smartregister.bidan.helper.view.RenderContactScreeningCardHelper;
import org.smartregister.bidan.helper.view.RenderPatientDemographicCardHelper;
import org.smartregister.bidan.helper.view.RenderPatientFollowupCardHelper;
import org.smartregister.bidan.helper.view.RenderPositiveResultsCardHelper;
import org.smartregister.bidan.helper.view.RenderServiceHistoryCardHelper;
import org.smartregister.bidan.model.Register;
...
import static org.smartregister.bidan.activity.BaseRegisterActivity.TOOLBAR_TITLE;
import static util.BidanConstants.ENKETO_FORMS;
...
public abstract class BasePatientDetailsFragment extends SecuredFragment implements View.OnClickListener {
    ...
    protected void renderPositiveResultsView(View view, Map<String, String> patientDetails) {
        RenderPositiveResultsCardHelper renderPositiveResultsHelper = new RenderPositiveResultsCardHelper(getActivity(), BidanApplication.getInstance().getResultsRepository());
    ...
    protected void renderDemographicsView(View view, Map<String, String> patientDetails) {
        ...
        RenderPatientDemographicCardHelper renderPatientDemographicCardHelper = new RenderPatientDemographicCardHelper(getActivity(), BidanApplication.getInstance().getResultDetailsRepository());
    ...
    protected void renderFollowUpView(View view, Map<String, String> patientDetails) {
        ...
        RenderPatientFollowupCardHelper renderPatientFollowupCardHelper = new RenderPatientFollowupCardHelper(getActivity(), BidanApplication.getInstance().getResultDetailsRepository());
    ...
    protected void renderServiceHistoryView(View view, Map<String, String> patientDetails) {
        RenderServiceHistoryCardHelper renderServiceHistoryHelper = new RenderServiceHistoryCardHelper(getActivity(), BidanApplication.getInstance().getResultDetailsRepository());
    ...
    protected void renderContactScreeningView(View view, Map<String, String> patientDetails) {
        RenderContactScreeningCardHelper renderContactScreeningHelper = new RenderContactScreeningCardHelper(getActivity(), BidanApplication.getInstance().getResultsRepository());
    ...
    protected void renderBMIHeightChartView(View view, Map<String, String> patientDetails) {
        ...
        RenderBMIHeightChartCardHelper renderBMIHeightChartCardHelper = new RenderBMIHeightChartCardHelper(getActivity(), BidanApplication.getInstance().getResultDetailsRepository());
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.add_contact:
                ((BasePatientDetailActivity) getActivity()).startFormActivity(ENKETO_FORMS.ADD_TB_CONTACT, patientDetails.get(BidanConstants.KEY.BASE_ENTITY_ID), formOverridesHelper.getAddContactFieldOverrides().getJSONString());
                break;
    ...
    protected void setupViews(View rootView) {
        ...
        ViewConfiguration config = BidanApplication.getJsonSpecHelper().getLanguage(Utils.getLanguage());
    ...
    protected void processViewConfigurations(View rootView) {
        try {
            String jsonString = BidanApplication.getInstance().getConfigurableViewsRepository().getConfigurableViewJson(getViewConfigurationIdentifier());
            if (jsonString == null) {
                renderDefaultLayout(rootView);

            } else {
                ViewConfiguration detailsView = BidanApplication.getJsonSpecHelper().getConfigurableView(jsonString);
                ...
                if (!views.isEmpty()) {
                    ...
                    for (org.smartregister.configurableviews.model.View componentView : views) {
                        ...
                        try {
                            ...
                            String jsonComponentString = BidanApplication.getInstance().getConfigurableViewsRepository().getConfigurableViewJson(componentView.getIdentifier());
                            ViewConfiguration componentViewConfiguration = BidanApplication.getJsonSpecHelper().getConfigurableView(jsonComponentString);
                            if (componentViewConfiguration != null) {

                                ConfigurableViewsHelper configurableViewsHelper = BidanApplication.getInstance().getConfigurableViewsHelper();
                                ...
                                View json2View = BidanApplication.getJsonSpecHelper().isEnableJsonViews() ? configurableViewsHelper.inflateDynamicView(componentViewConfiguration, viewParent, fallbackView, componentView.isVisible()) : fallbackView;
                                if (componentView.isVisible()) {
                                    json2View.setTag(R.id.VIEW_CONFIGURATION_ID, getViewConfigurationIdentifier());

                                    if (!BidanApplication.getJsonSpecHelper().isEnableJsonViews()) {
